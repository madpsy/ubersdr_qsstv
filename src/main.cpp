...
#ifdef HEADLESS
// ---------------------------------------------------------------------------
// Headless build — QCoreApplication, no GUI, reads audio from stdin
// ---------------------------------------------------------------------------
...
int main(int argc, char **argv)
{
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--output-dir" && i + 1 < argc)
      outputDir = QString(argv[++i]);
    else if (arg == "--events-fd" && i + 1 < argc)
      eventsFd = QString(argv[++i]).toInt();
    else if (arg == "--freq-label" && i + 1 < argc)
      freqLabel = QString(argv[++i]);
  }

  if (outputDir.isEmpty())
    outputDir = QString(getenv("HOME")) + "/qsstv/rx_sstv";

  globalInit();
  headlessController *ctrl = new headlessController(outputDir, eventsFd, freqLabel);
  ctrl->init();
  ctrl->startRunning();
  int result = app.exec();
  globalEnd();
...
#else
// ---------------------------------------------------------------------------
// GUI build — QApplication, splash screen, mainWindow
// ---------------------------------------------------------------------------
