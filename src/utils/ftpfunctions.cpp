...
bool ftpFunctions::checkUpload(QString fn, QString rfn)
{
  ...
  QCoreApplication::postEvent(ftpThr,ftpUpl);
  checkWait(true);
...
bool ftpFunctions::checkRemove(QString rfn)
{
  ...
  QCoreApplication::postEvent(ftpThr,ftpRemove);
  checkWait(true);
  return lastError==FTPOK;
}
...
void ftpFunctions::downloadFile(QString source, QString destination,bool wait,bool closeWhenDone)
{
  ...
  QCoreApplication::postEvent(ftpThr,ftpDwnl);
  checkWait(wait);
}
...
void ftpFunctions::mremove(QString mask,bool wait,bool closeWhenDone)
{
  ...
  QCoreApplication::postEvent(ftpThr,ftpList);
  checkWait(wait);
}
...
void ftpFunctions::rename(QString source, QString destination,bool wait,bool closeWhenDone)
{
  ...
  QCoreApplication::postEvent(ftpThr,ftpRename);
  checkWait(wait);
}
...
void ftpFunctions::changePath(QString source,bool wait)
{
  ...
  QCoreApplication::postEvent(ftpThr,ftpCd);
  checkWait(wait);
}
  ...
  if(ftpThr)
    {
      ...
      QCoreApplication::postEvent(ftpThr,ftpDisconnect);
      busy=false;
    }
}
...
#else
        (void)stmb; // no dispatcher in headless
#endif
      }
}
