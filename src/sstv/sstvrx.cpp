      ...
      if(syncProcPtr==NULL)
        {
          ...
          QCoreApplication::postEvent( dispatcherPtr, stce );  // Qt will delete it when done
          advanceBuffers();
          break; // no sync
        }
...
//      ri=bufferVideoDemod.getReadIndex();
    ...
    case SYNCLOST:
      saveImage();
      if (syncProcPtr->currentModePtr->receivedLines() == 0) {
        // No image data decoded yet — safe to rewind to the last good sync
        // position and retry.  This handles false locks on noise bursts.
      ...
      // else: partial image already saved.  Rewinding would re-process
      // mid-image audio as a new decode attempt, producing a spurious
      // second image containing only the bottom portion of the transmission.
      // Skip the rewind and go straight back to HUNTING.
      syncProcPtr->resetRetraceFlag();
      resetParams(true);
      break;
    case WAIT:
...
//      endce = new endImageSSTVRXEvent(getSSTVModeNameShort(syncProcPtr->getMode()));
  ...
  endce->waitFor(&done);
  QCoreApplication::postEvent(dispatcherPtr, endce);
