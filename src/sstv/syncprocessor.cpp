...
void syncProcessor::init()
{
  ...
  QCoreApplication::postEvent(dispatcherPtr, ce);
...
void syncProcessor::extractSync()
{
  ...
  // DEBUG: periodically log volume statistics to diagnose sync detection
  // Use separate counters for narrow vs wide so both get logged
  static int extractSyncDbgCountWide = 0;
  static int extractSyncDbgCountNarrow = 0;
  int &extractSyncDbgCount = detectNarrow ? extractSyncDbgCountNarrow : extractSyncDbgCountWide;
  if (++extractSyncDbgCount % 120 == 0) {
    int belowMinVol=0, syncOnCount=0;
    double maxInputVol=0, maxSyncVol=0, avgInputVol=0, avgSyncVol=0;
    for(int j=0;j<RXSTRIPE;j++) {
      if(inputVolumePtr[j]<sensitivityArray[sensitivity].minVolume) belowMinVol++;
      if(inputVolumePtr[j]>maxInputVol) maxInputVol=inputVolumePtr[j];
      avgInputVol+=inputVolumePtr[j];
      if(syncVolumePtr[j]>maxSyncVol) maxSyncVol=syncVolumePtr[j];
      avgSyncVol+=syncVolumePtr[j];
      if(inputVolumePtr[j]>=sensitivityArray[sensitivity].minVolume
         && syncVolumePtr[j]>sensitivityArray[sensitivity].onRatio*inputVolumePtr[j])
        syncOnCount++;
    }
    avgInputVol/=RXSTRIPE;
    avgSyncVol/=RXSTRIPE;
  }
...
void syncProcessor::slotNewCall(QString call)
{
  ...
  // Mirror the guard in checkSyncArray(): only trigger retrace if we are
  // actively decoding an image.  When the FSK-ID arrives as a preamble
  // *before* the image (e.g. live SDR streams), setting retraceFlag here
  // would abort the decode the instant it starts.
...
bool  syncProcessor::createModeBase()
{
  ...
  if (currentMode!=NOTVALID)
  {
    ...
    // Clear any stale retrace flag that may have been set by the FSK-ID preamble
    // arriving in the same audio buffer as the VIS code.  At this point we are
    // starting a fresh image decode, so a retraceFlag=true is always a false
    // positive that would cause saveImage() to fire immediately with 0 lines.
    retraceFlag = false;
    startImageRXEvent* ce = new startImageRXEvent(QSize(currentModePtr->imagePixels(),currentModePtr->imageLines()), currentMode);
    ce->waitFor(&done);
    QCoreApplication::postEvent(dispatcherPtr, ce);
    while(!done)
    {
