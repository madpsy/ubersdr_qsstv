...
# ─── Headless build ──────────────────────────────────────────────────────────
# Build with: qmake CONFIG+=headless qsstv.pro && make
headless {
    DEFINES     += HEADLESS
    TARGET       = qsstv-headless

    # Keep gui/widgets so config .cpp files (which inherit QWidget) compile
    # QJsonObject/QJsonDocument are part of QtCore since Qt 5.0 — no extra module needed
    QT           = core gui widgets network xml

    # No pkg-config for pulse/alsa/hamlib — stdin audio only
    CONFIG      -= link_pkgconfig
    PKGCONFIG    =
    LIBS         = -lfftw3f -lfftw3 -lopenjp2

    # Add headless include path + standard openjpeg headers (from libopenjp2-7-dev)
    INCLUDEPATH += headless /usr/include/openjpeg-2.5

    # ── Sources: RX-only subset + headless-specific ──
    ...
    SOURCES = \
        ...
        mainwidgets/rxfunctions.cpp \
        config/baseconfig.cpp \
        headless/configglobals.cpp \
        headless/headlesscontroller.cpp \
        headless/headlessrxcontroller.cpp \
        headless/headlessdispatcher.cpp \
        headless/headlessevents.cpp
    ...
    HEADERS = \
        ...
        utils/qftp.h \
        config/baseconfig.h \
        headless/headlesscontroller.h \
        headless/headlessrxcontroller.h \
        headless/headlessimagebuffer.h \
        headless/headlessdispatcher.h \
        headless/headlessevents.h

    # No .ui files in headless mode
    FORMS =

    # No resources needed
    RESOURCES =

    # No install target for headless binary
    INSTALLS =
}
