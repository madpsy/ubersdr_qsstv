...
bool  wavIO::openFileForRead(QString fname,bool ask)
{
...
#else
      Q_UNUSED(fname)
      return false; // file dialogs not supported in headless mode
#endif
    }
  else
    {
  ...
  if(!inopf.open(QIODevice::ReadOnly))
    {
#ifndef HEADLESS
      displayMBoxEvent *stmb = new displayMBoxEvent("WAVIO Error", QString("Unable to open file %1\nError: %2").arg(fname).arg(strerror(errno)));
      QApplication::postEvent( dispatcherPtr, stmb );
#else
      qWarning() << "WAVIO Error: Unable to open file" << fname << strerror(errno);
#endif
      return false;
    }
  reading=true;
  if(inopf.read(&waveHeader.chunkID[0],sizeof(sWave))!=sizeof(sWave))
    {
#ifndef HEADLESS
      displayMBoxEvent *stmb = new displayMBoxEvent("WAVIO Error", QString("Invalid format"));
      QApplication::postEvent( dispatcherPtr, stmb );
#else
      qWarning() << "WAVIO Error: Invalid format";
#endif
      closeFile();
      return false;
    }
...
#ifndef HEADLESS
      displayMBoxEvent *stmb = new displayMBoxEvent("WAVIO Error", QString("Invalid Header Format"));
      QApplication::postEvent( dispatcherPtr, stmb );
#else
      qWarning() << "WAVIO Error: Invalid Header Format";
#endif
      closeFile();
      return false;
    }
...
#ifndef HEADLESS
      displayMBoxEvent *stmb = new displayMBoxEvent("WAVIO Error", QString("Format not supported"));
      QApplication::postEvent( dispatcherPtr, stmb );
#else
      qWarning() << "WAVIO Error: Format not supported";
#endif
      closeFile();
      return false;
    }
...
#ifndef HEADLESS
      displayMBoxEvent *stmb = new displayMBoxEvent("WAVIO Error", QString("Number of samples  is zero"));
      QApplication::postEvent( dispatcherPtr, stmb );
#else
      qWarning() << "WAVIO Error: Number of samples is zero";
#endif
      closeFile();
      return false;
    }
