        ...
        case RXRUNNING:
          if((count=soundIOPtr->rxBuffer.count())<RXSTRIPE)
          ...
          else
            {
              ...
              // DEBUG: log audio flow every ~10 seconds
              static int rxFlowDbg = 0;
              if (++rxFlowDbg % 120 == 0) {
                addToLog(QString("RXFLOW_DBG: rxBytes=%1 bufCount=%2 mode=%3 rxState=%4")
                  .arg(rxBytes).arg(soundIOPtr->rxBuffer.count())
                  .arg(transmissionModeIndex).arg(rxState),LOGALL);
              }
