...
const (
	...
	//
	// The server emits it only when the client asked for a margin on the
	// connect URL, and this addon receives demodulated audio and never does, so
	// no packet it sees carries this profile. It is implemented so the decoder
	// reads the whole of the version 4 wire format rather than the part this
	// one caller happens to need, and so the server's own conformance fixture
	// for it can be run against this copy.
