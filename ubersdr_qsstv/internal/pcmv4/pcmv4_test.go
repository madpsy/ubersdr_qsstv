...
// mode a min_margin request asks the server for: profile 2, where a shift byte
// leads the body and the samples come back shifted left by it.
//
// This addon receives demodulated audio and never asks for the mode, so nothing
// it connects to emits profile 2. The fixture is here because the profile is
// part of the version 4 wire format this decoder claims to read: it covers the
// paths that exist nowhere else -- a shift that changes as the margin does, a
// silent packet carrying no shift at all, an escape that carries one, and the
// profile switching to plain IQ and back when the margin goes to lossless.
