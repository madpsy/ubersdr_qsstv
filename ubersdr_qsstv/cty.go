...
// If filename is empty or "-", the embedded cty/cty.dat is used.
func InitCTYDatabase(filename string) error {
	...
	if filename != "" {
		...
		scanner = bufio.NewScanner(strings.NewReader(string(embeddedCTYData)))
	}
...
func parseEntityLine(line string) (*CTYEntity, error) {
	...
	// CTY.DAT stores longitude in "degrees West" format — negate to get East-positive.
